...
async function main() {
    ...
    // Note: We use executeRaw to perform SQL actions that Prisma doesn't support natively
    try {
    ...
    try {
        // Enable real-time for ProductVariant table
        await prisma.$executeRawUnsafe(`
      begin;
        -- remove the table from the publication if it exists
        alter publication supabase_realtime drop table if exists "ProductVariant";
